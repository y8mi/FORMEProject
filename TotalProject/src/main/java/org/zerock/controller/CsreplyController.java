...
public class CsreplyController {
	...
	@GetMapping(value = "/pages/{csbno}/{page}", 
	produces = {
			 MediaType.APPLICATION_XML_VALUE,
			 MediaType.APPLICATION_JSON_UTF8_VALUE })
	public ResponseEntity<List<CsreplyVO>> getList(
			@PathVariable("page") int page,
			@PathVariable("csbno") Long csbno) {
	
	
		log.info("getList.................");
		PagingCriteria cri = new PagingCriteria(page,10);
		log.info(cri);
		
		return new ResponseEntity<>(service.getList(cri, csbno), HttpStatus.OK);
	}
	
	@GetMapping(value = "/{rno}", 
			produces = { MediaType.APPLICATION_XML_VALUE, 
					     MediaType.APPLICATION_JSON_UTF8_VALUE })
	public ResponseEntity<CsreplyVO> get(@PathVariable("rno") Long rno) {
